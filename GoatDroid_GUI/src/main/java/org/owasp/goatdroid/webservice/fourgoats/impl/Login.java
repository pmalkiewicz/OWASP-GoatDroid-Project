...
public class Login {
    ...
    static public LoginBean validateCredentials(String userName, String password) {
        ...
        try {
            ...
            if (errors.size() == 0) {
                dao.openConnection();
                boolean accountLocked = !dao.checkLockout(userName);
                if (accountLocked) {
                    boolean lockoutShouldBeCleared = dao.checkLastUnsuccessfulLogin(userName);
                    if (lockoutShouldBeCleared) {
                ...
                if (validCredentials && !accountLocked) {
                    ...
                    errors.add(Constants.ACCOUNT_LOCKOUT_MSG);
                } else {
                    errors.add(Constants.LOGIN_CREDENTIALS_INVALID);
                    dao.incrementLockoutNumber(userName);
                    dao.setLastUnsuccessfulLogin(userName);
                }
            }
        } catch (Exception e) {
